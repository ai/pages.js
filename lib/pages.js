...
;(function($, undefined) {
  ...
  var self = window.Pages = {
    ...
    enable: function() {
      ...
      $(window).on('popstate.pages', function() {
        if ( self._lastUrl != (history.location || location).pathname ) {
          self.open((history.location || location).pathname, { from: 'popstate' });
        }
      });
    ...
    open: function(url, data) {
      if ( (history.location || location).pathname != url ) {
    ...
    page: function(url, base) {
      ...
      var selector = self.pagesSelector + '[data-url="' + url + '"]';
    ...
    _findCurrent: function() {
      return $(self.pagesSelector + ':visible:first', self._doc)
    },
    ...
    _openLink: function(link) {
      ...
      self.open(path, data);
      if ( hash && (history.location || location).hash != hash ) {
        (history.location || location).hash = hash;
      }
      return false;
    },
  ...
  $(document).ready(function() {
    self._lastUrl = (history.location || location).pathname;
    self.init();
    if ( !self.disabled ) {
      self.enable();
    }
